"""Pseudo object detection visualization for EuroSAT.

EuroSAT has no official bounding boxes. This script extracts heuristic
regions and draws bounding boxes for presentation/analysis only.
"""

import numpy as np
import matplotlib.pyplot as plt
from matplotlib.patches import Rectangle
from torchvision.datasets import EuroSAT
from torchvision import transforms

from config import DATA_DIR
from config import EUROSAT_CLASSES
from config import RESULTS_DIR


OUTPUT_DIR = RESULTS_DIR / "pseudo_object_detection"
OUTPUT_DIR.mkdir(exist_ok=True)


def create_detection_mask(image_np, class_name):
    """Create class-dependent heuristic foreground mask."""
    r = image_np[:, :, 0]
    g = image_np[:, :, 1]
    b = image_np[:, :, 2]
    brightness = (r + g + b) / 3

    if class_name in ["River", "SeaLake"]:
        return (b > r) & (b > g) & (brightness > 0.16)

    if class_name == "Forest":
        return (g > r) & (g > b) & (brightness > 0.10)

    if class_name == "Highway":
        gray = (np.abs(r - g) < 0.08) & (np.abs(g - b) < 0.08)
        return gray & (brightness > 0.30)

    if class_name in ["Residential", "Industrial"]:
        gray = (np.abs(r - g) < 0.12) & (np.abs(g - b) < 0.12)
        return gray & (brightness > 0.25)

    vegetation = (g > r * 0.85) & (g > b * 0.85)
    return vegetation & (brightness > 0.12)


def mask_to_bbox(mask):
    """Convert binary mask to one bounding box."""
    y_indices, x_indices = np.where(mask)

    if len(x_indices) == 0 or len(y_indices) == 0:
        return None

    x_min = int(x_indices.min())
    x_max = int(x_indices.max())
    y_min = int(y_indices.min())
    y_max = int(y_indices.max())

    if x_max <= x_min or y_max <= y_min:
        return None

    return x_min, y_min, x_max, y_max


def main():
    dataset = EuroSAT(
        root=str(DATA_DIR),
        download=True,
        transform=transforms.ToTensor(),
    )

    examples = {}

    for image, label in dataset:
        class_name = dataset.classes[label]

        if class_name not in examples:
            examples[class_name] = image

        if len(examples) == len(EUROSAT_CLASSES):
            break

    for class_name in EUROSAT_CLASSES:
        image = examples[class_name]
        image_np = image.permute(1, 2, 0).numpy()
        mask = create_detection_mask(image_np, class_name)
        bbox = mask_to_bbox(mask)

        fig, axes = plt.subplots(1, 3, figsize=(16, 5))
        fig.suptitle(f"Pseudo Object Detection: {class_name}", fontsize=18, fontweight="bold")

        axes[0].imshow(image_np, interpolation="nearest")
        axes[0].set_title("Original Image")
        axes[0].axis("off")

        axes[1].imshow(mask, cmap="autumn", interpolation="nearest")
        axes[1].set_title("Detection Mask")
        axes[1].axis("off")

        axes[2].imshow(image_np, interpolation="nearest")
        axes[2].set_title("Bounding Box")
        axes[2].axis("off")

        if bbox is not None:
            x_min, y_min, x_max, y_max = bbox
            rectangle = Rectangle(
                (x_min, y_min),
                x_max - x_min,
                y_max - y_min,
                linewidth=3,
                edgecolor="red",
                facecolor="none",
            )
            axes[2].add_patch(rectangle)
            axes[2].text(
                x_min,
                max(y_min - 2, 0),
                class_name,
                color="red",
                fontsize=12,
                fontweight="bold",
                backgroundcolor="white",
            )

        plt.tight_layout()
        save_path = OUTPUT_DIR / f"{class_name}_pseudo_detection.png"
        plt.savefig(save_path, dpi=300, bbox_inches="tight")
        plt.close()

        print(f"Saved: {save_path}")


if __name__ == "__main__":
    main()
